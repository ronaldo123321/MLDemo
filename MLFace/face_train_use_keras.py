...
class Model:
    ...
    def build_model(self, dataset, nb_classes=3):
        ...
        #第一个卷积层包含32个卷积核，每个卷积核大小为3x3，border_mode值为“same”意味着我们采用保留边界特征的方式滑窗，而值“valid”则指定丢掉边界像素
        #input_shape的值为(64,64,3)，来自Dataset类，代表64x64的彩色RGB图像
        self.model.add(Convolution2D(32, 3, 3, border_mode='same',input_shape=dataset.input_shape))  # 1 2维卷积层

        #relu(修正线性单元函数) relu函数是小于0输出为0，大于0等于原值|| 激活函数可以将简单的线性分类变成复杂的非线性分类
        self.model.add(Activation('relu'))  # 2 激活函数层

        self.model.add(Convolution2D(32, 3, 3))  # 3 2维卷积层
        self.model.add(Activation('relu'))  # 4 激活函数层

        #池化层存在的目的是缩小输入的特征图，简化网络计算复杂度，同时进行特征压缩，突出主要特征。此处采用了最大值池化法
        self.model.add(MaxPooling2D(pool_size=(2, 2)))  # 5 池化层
        #随机断开一定百分比的输入神经元连接，以防止过拟合。Dropout层将有意识的随机减少模型参数，让模型变得简单，而越简单的模型越不容易产生过拟合
        ...
        #截至到Flatten层之前在网络中流动的数据还是多维的，经过多次卷积，池化，Dropout之后，到这里可以进入全连接层做最后
        # 处理。全连接层要求输入的数据必须是一维的，因此，必须把输入数据'压扁'成一维后才能进入全连接层，不需要任何输入参数
        self.model.add(Flatten())  # 13 Flatten层
        #全连接层的作用就是用于分类或回归，对我们来说就是分类。
        ...
        # 训练模型
    def train(self, dataset, batch_size=30, nb_epoch=10, data_augmentation=True):
        sgd = SGD(lr=0.01, decay=1e-6, #SGD 是梯度下降的一个变体。它并不是用全体样本计算 e 的梯度，而是每次迭代使用随机选择的一部分样本来计算
                  momentum=0.9, nesterov=True)  # 采用SGD+momentum的优化器进行训练，首先生成一个优化器对象
        #lr学习效率   decay是每次更新后学习效率的衰减值    参数momentum即用于指定在多大程度上保留原有方向，其值为0~1之间的浮点数。一般来说，选择一个在0.5~0.9之间的数即可。
...
if __name__ == '__main__':
    dataset = Dataset('F:\pyworkspace\data')
    dataset.load()
